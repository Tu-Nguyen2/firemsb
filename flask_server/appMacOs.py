...
cred = credentials.Certificate("../msbuddy-69e38-firebase-adminsdk-h1gp2-9599110e0d.json") # this must be from downloading the firebase SDK private key, ensure the file is in gitignore
...
#comment out the path if it is not explicitly used, in windows had to do this, unsure of why the behavior has this issue
...
def convert_to_mp4(input_file, output_file=None):
    ...
    try:
        ...
        ffmpeg_command = [
            # ffmpeg_path,        #make sure that changes here are reflected if switching OS or a path issue
